...
illustrates the four steps of the Island clustering algorithm:
  ...
  fig1_layout.png         — full HyCal sectors + module types
  fig2_single_cluster.png — DFS grouping, seed, log-weighted centroid
  fig3_split.png          — two overlapping showers, profile-based split
  fig4_params.png         — log_weight_thres effect + shower-depth curves
...
ax.grid(False)
fig.tight_layout()
fig.savefig(HERE / 'fig1_layout.png', dpi=130)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(HERE / 'fig2_single_cluster.png', dpi=130)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(HERE / 'fig3_split.png', dpi=130)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(HERE / 'fig4_params.png', dpi=130)
plt.close(fig)
